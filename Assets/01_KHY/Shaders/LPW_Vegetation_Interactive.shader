...
Shader "DearBrave/LPW Vegetation Interactive"
{
	Properties
	{
	...
	SubShader
	{
		...
		// --- Player Interaction (Global) ---
		float4 _GrassPlayerPos;       // xyz = world pos, w = unused
		float _GrassInteractionRadius; // default 1.5
		float _GrassInteractionStrength; // default 1.5

		float3 ApplyGrassInteraction(float3 vertexWorldPos, float3 positionOS, float vertexY)
		{
			float dist = distance(vertexWorldPos.xz, _GrassPlayerPos.xz);
			float radius = max(_GrassInteractionRadius, 0.01);
			float influence = saturate(1.0 - dist / radius);
			influence = influence * influence * influence; // cubic falloff

			// 높이 비율 (뿌리=0, 끝=1)
			float heightFactor = saturate(vertexY / max(radius * 0.3, 0.05));

			// 밀어내는 방향
			float2 pushDir = normalize(vertexWorldPos.xz - _GrassPlayerPos.xz + float2(0.001, 0.001));

			// 꺾이는 각도 (라디안) - 최대 80도까지 눕힘
			float bendAngle = _GrassInteractionStrength * influence * heightFactor * 1.4;
			bendAngle = min(bendAngle, 1.4); // ~80도 제한

			// 뿌리 기준 회전 - 높이를 유지하면서 원호로 이동
			float sinA = sin(bendAngle);
			float cosA = cos(bendAngle);

			// 원래 높이(vertexY) 기준으로 회전
			float newHeight = vertexY * cosA - vertexY * 0.0; // 높이 줄어듦
			float pushAmount = vertexY * sinA; // 옆으로 밀림

			float3 push = float3(
				pushDir.x * pushAmount,
				-(vertexY - vertexY * cosA), // 높이 차이만큼 아래로
				pushDir.y * pushAmount
			);
			return push;
		}
		...
		Pass
		{
			...
			PackedVaryings VertexFunction( Attributes input  )
			{
				...
				// Player interaction
				float3 worldPosForInteraction = mul( GetObjectToWorldMatrix(), float4( input.positionOS.xyz + vertexValue, 1.0 ) ).xyz;
				vertexValue += mul( GetWorldToObjectMatrix(), float4( ApplyGrassInteraction( worldPosForInteraction, input.positionOS.xyz, input.positionOS.xyz.y ), 0.0 ) ).xyz;
